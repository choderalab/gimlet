...
ifs.open('gdb9.sdf')
mols = [next(ifs.GetOEGraphMols()) for _ in range(2000)]
...
point = {'D_V': 64, 'D_E': 64, 'D_U': 64, 'phi_e_0': 64, 'phi_e_a_0': 'leaky_relu', 'phi_e_a_1': 'elu', 'phi_v_0': 128, 'phi_v_a_0': 'tanh', 'phi_v_a_1': 'elu', 'phi_u_0': 256, 'phi_u_a_0': 'relu', 'phi_u_a_1': '
elu', 'f_r_0': 32, 'f_r_1': 32, 'f_r_a_0': 'relu', 'f_r_a_1': 'leaky_relu', 'learning_rate': 1e-05}
    ...
    class f_r(tf.keras.Model):
        ...
        def call(self,
            ...
            h_t = tf.concat(
                ...
                axis=1)
            # (n_atoms, n_atoms)
            q = tf.squeeze(self.d_q_1(
                    self.d_q_0(
                        h_v)))

            # (n_atoms, n_atoms)
            sigma = tf.squeeze(tf.exp(self.d_sigma_1(
                    self.d_sigma_0(
                        h_v))))


            # (n_atoms, n_atoms)
            epsilon = tf.squeeze(tf.exp(self.d_epsilon_1(
                    self.d_epislon_0(
                        h_v))))
...
def obj_fn(point):
        ...
        for atoms, adjacency_map, angles, bonds, torsions, particle_params in ds_tr:
            ...
            with tf.GradientTape() as tape:
                ...
                angle_loss = tf.keras.losses.MAE(
                    ...
                    tf.reshape(tf.stack([a_l, a_k], axis=1), [-1]))

                bond_loss = tf.keras.losses.MAE(
                    tf.reshape(bonds[:, 2:], [-1]),
                    tf.reshape(tf.stack([e_l, e_k], axis=1), [-1]))
    ...
    np.save('bond_l_tr_true', bond_true_tr[1:, 0].numpy())
    np.save('bond_l_tr_pred', bond_pred_tr[1:, 0].numpy())
    print('bond k tr')
    print(metrics.r2_score(bond_true_tr[1:, 1], bond_pred_tr[1:, 1]))
    np.save('bond_k_tr_true', bond_true_tr[1:, 1].numpy())
    np.save('bond_k_tr_pred', bond_true_tr[1:, 1].numpy())
    print('angle l tr')
    print(metrics.r2_score(angle_true_tr[1:, 0], angle_pred_tr[1:, 0]))
    np.save('angle_l_tr_true', angle_true_tr[1:, 0].numpy())
    np.save('angle_l_tr_pred', angle_pred_tr[1:, 0].numpy())
    print('angle k tr')
    print(metrics.r2_score(angle_true_tr[1:, 1], angle_pred_tr[1:, 1]))
    np.save('angle_k_tr_true', angle_true_tr[1:, 1].numpy())
    np.save('angle_k_tr_pred', angle_pred_tr[1:, 1].numpy())
    print('q tr')
    print(metrics.r2_score(atom_true_tr[1:, 0], atom_pred_tr[1:, 0]))
    np.save('q_tr_true', atom_true_tr[1:, 0].numpy())
    np.save('q_tr_pred', atom_pred_tr[1:, 0].numpy())
    print('sigma tr')
    print(metrics.r2_score(atom_true_tr[1:, 1], atom_pred_tr[1:, 1]))
    np.save('sigma_tr_true', atom_true_tr[1:, 1].numpy())
    np.save('sigma_tr_pred', atom_pred_tr[1:, 1].numpy())
    print('epsilon tr')
    print(metrics.r2_score(atom_true_tr[1:, 2], atom_pred_tr[1:, 2]))
    np.save('epsilon_tr_true', atom_true_tr[1:, 2].numpy())
    np.save('epsilon_tr_pred', atom_pred_tr[1:, 2].numpy())
